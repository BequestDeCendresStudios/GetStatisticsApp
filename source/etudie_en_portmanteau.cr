...
def graviteformee(a, b)
  ...
  "Le #{a} es uni avec #{b}: #{new_word[0]}#{new_word[1]}"
end

def analyze_graviteformee(a, b, c, d, e, f)  
  2.times do
    get_statistics(:first_,        graviteformee(a, b),
                   :second_second, graviteformee(c, d),
                   :third_word,    graviteformee(e, f))
...
word_index = [[
  ...
  [ :abandon, :abandon,   :abbatage ],
  [ :abandon, :abandon, :abattement ],
], [
  [ :hoteru, :hoteru,  :mise ],
  [ :hoteru, :hoteru, :michi ],
  [ :hoteru, :hoteru,  :supa ],
...
## Specific matrix
french_words   = word_index[0]
japanese_words = word_index[1]

## Using graviteformee
## Words that form under gravite that dissapear without an atmosphere, or "context".

analyze_graviteformee(graviteformee(french_words[0][2], japanese_words[0][2]), graviteformee(french_words[0][1], japanese_words[0][2]),# Mise De Abandonner
                      graviteformee(french_words[1][2], japanese_words[1][2]), graviteformee(french_words[1][1], japanese_words[1][2]),# Abbatage De Michi
                      graviteformee(french_words[2][2], japanese_words[2][2]), graviteformee(french_words[2][1], japanese_words[2][2])) # Abattement De Supa
